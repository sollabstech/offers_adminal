...
export default function EditProductPage() {
  ...
  const { products, vendors, updateProduct } = useAdminStore();
  ...
  if (!product) {
    return (
      <div className="flex flex-col items-center justify-center h-64 text-slate-400">
        ...
        <Link href="/products" className="mt-3 text-orange-500 hover:underline text-sm">Back to products</Link>
      </div>
    );
  }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    setSaving(true);
    const isOfferss = form.vendorId === "__offerss__";
    ...
    await updateProduct(id, {
      ...
      vendorId: isOfferss ? "__offerss__" : form.vendorId,
      vendorName: isOfferss ? "Offerss.com" : (vendor?.name ?? product.vendorName),
  ...
  return (
    <div className="max-w-2xl">
      <div className="flex items-center gap-3 mb-6">
        <Link href="/products" className="w-9 h-9 flex items-center justify-center rounded-xl bg-white border border-slate-200 hover:bg-slate-50 transition-colors">
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-6 space-y-4">
          ...
          <div>
            ...
            <select required value={form.vendorId} onChange={(e) => set("vendorId", e.target.value)} className="input-field">
              <option value="__offerss__">⭐ Offerss.com (Admin — sell directly)</option>
              {vendors.map((v) => (
        ...
        <div className="flex gap-3">
          <Link href="/products" className="flex-1 py-3 text-center rounded-xl border border-slate-200 text-slate-700 font-medium hover:bg-slate-50 transition-colors">
            Cancel
          </Link>
